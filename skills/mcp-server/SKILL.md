...
> **Provider 层**: `AgentChat-WebExtended`（单源真相）
> **安全策略**: 永不关闭用户 Chrome
...
AgentChat MCP Server (本 skill, ~200 行)
    │
    │  child_process.spawn('node', ['AgentChat-WebExtended/index.js', ...])
    ▼
AgentChat-WebExtended (8 个 Provider)
    │
...
| `AgentChat-WebExtended` | Provider 实现（同一仓库内） |
